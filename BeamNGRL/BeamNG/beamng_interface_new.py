...
from beamngpy.sensors import Lidar, Camera, Electrics, Timer, Damage
import threading
...
def get_beamng_default(
    ...
    enable_traffic = traffic_config["enable"] if traffic_config is not None and "enable" in traffic_config else False
    bng = beamng_interface(BeamNG_path=beamng_path, remote=remote, host_IP=host_IP, enable_traffic=enable_traffic)
...
class beamng_interface():
    ...
    def send_ctrl(self, action, speed_ctrl=False, speed_max=1, Kp=1, Ki=1, Kd=0, FF_gain=1):
        ...
        if speed_ctrl:
            speed_err = th - (self.avg_wheelspeed / speed_max)
            th, self.whspd_error_sigma, self.whspd_error_diff = self.scaled_PID_FF(
                Kp, Ki, Kd, FF_gain, th, speed_err, self.whspd_error_sigma, self.whspd_error_diff, self.last_whspd_error
            )
            self.last_whspd_error = speed_err
            th = np.clip(th, -1, 1)
        br = 0
        ...
        if th < 0:
            br = -th
            th_out = 0
        # Update control call as per the new API if needed.
        self.vehicle.control(throttle=th_out, brake=br, steering=st)
